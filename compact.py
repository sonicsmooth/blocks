...
# def sort_rects_y(rects, ids, key, reverse=False):
#     # from list of rects, sort from top to bottom
#     chosen_rects = [r for id, r in rects.items() if id in ids]
#     sorted_rects = sorted(chosen_rects, key=lambda x: x['pos'].y())
#     sorted_ids = [r['id'] for r in sorted_rects]
#     return sorted_ids
...
def update_graph_xleft(rects):
    ...
    for edge in sorted_edges:
        ...
        if active_edges:
            ...
            tmp_edges['sorted'] = sort_rects(rects, tmp_edges['ids'], key=lambda x: x['pos'].x(), reverse=False)
            ...
            active_edges.append(tmp_edges)
    return compose_graph(active_edges, rects, 'x')
...
def update_graph_yup(rects):
    ...
    for edge in sorted_edges:
        ...
        if active_edges:
            ...
            tmp_edges['sorted'] = sort_rects(rects, tmp_edges['ids'], key=lambda x: x['pos'].y())
            ...
            active_edges.append(tmp_edges)
    return compose_graph(active_edges, rects, 'y')


def longest_path_bellman_ford_xleft(graph, minx):
    MINDIST = 0
    edges = graph.keys()
    nodes = set()
    for edge in edges:
    ...
    dist[0] = minx  # Assuming 0 is the start node
...
#             if dist[frm] != float('-inf') and dist[frm] + weight > dist[to]:
#                 dist[to] = dist[frm] + weight
#     newdist = [maxx - dst for dst in dist]
#     return newdist
